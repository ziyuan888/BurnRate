...
- **Glassmorphism 毛玻璃风格** Popover 仪表盘
  - 顶部标题栏显示当前时间，快捷图标按钮（刷新 / 设置 / 关闭）；右上角 `关闭` 会直接收起当前 popover
...
- **Kimi 浏览器 Cookie 自动导入**：支持从 Chrome / Edge / Brave / Arc 自动读取 Kimi 登录态
- **动态菜单栏图标**：22×22 双条 meter，实时反映最紧张的 5 小时窗口和 7 天额度使用比例
- **Pace 使用节奏分析**：Kimi 7 天额度显示使用节奏是否正常、超支预估或结余空间
- **刷新频率预设档位**：手动 / 1m / 2m / 5m / 15m / 30m 一键切换
...
- Kimi 设置页支持点击「从浏览器自动导入」直接读取本地 Chromium 系浏览器的 Cookie，无需手动抓包
...
- 针对 Kimi 的 7 天额度窗口，BurnRate 会对比你的实际用量和"均匀消耗预算"
- 如果使用节奏正常，会显示 `使用节奏正常`
- 如果超支，会显示 `超支 X% · 预计 Y 小时后耗尽`
- 如果使用偏慢，会显示 `结余 X% · 使用节奏偏慢`
- 窗口刚开始的前 3% 时间内（约 5 小时）不显示 Pace，避免早期数据噪音

刷新策略：

- 支持 `手动刷新`、`1 分钟`、`2 分钟`、`5 分钟`、`15 分钟`、`30 分钟`
- 选择手动刷新后，后台不会自动轮询 API，仅在点击"立即刷新"时更新
- 默认档位为 `5 分钟`
...
**方式一：自动导入（推荐）**
1. 在 BurnRate 设置页找到 `Kimi`
2. 点击「从浏览器自动导入」按钮
3. 应用会自动扫描 Chrome / Edge / Brave / Arc 的 Cookie 并提取登录态
...
- 如果 token 过期、退出登录或控制台风控失效，BurnRate 会刷新失败，此时重新点击「从浏览器自动导入」或手动抓取一次即可
- 这是基于当前控制台请求抓取的接入方式，不属于公开稳定 API，后续如果 Kimi 改了控制台接口，获取步骤可能需要跟着调整
- **Kimi 的 5 小时频限采用滑动窗口机制**：BurnRate 会直接显示 API 返回的实时 `resetTime`，不会基于历史记录做固定周期推算
